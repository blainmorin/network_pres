...
Using the Buzzfeed data, I construct a an employment network that shows the flow between agencies. In Figure 2, I present thinned network where the different Agencies are the nodes and the edges represent an instance where more than 2\% of an agencies employees left to another between the years 1980 and 1990. These edges are binary and are limited to agencies with greater than 5,000 employees. These parameters were chosen to simplify the network for preliminary modeling purposes. I chose a 2\% threshold because it seemed to represent a significant flow. 
...
* The proportional budget increase of the agency over the last ten years. For example, if an agency's budget grew by 100\% over the past ten year, then this variable is coded as a 1. For this first term, I am expecting that an increase in budget increases the ability of an agency to poach from other agencies. Thus, I expect this variable to be positively correlated with the number of in-edges at an agency.
...
Next, I incorporate endogenous dependencies in the model. For now, I include 1 endogenous covaiate:

* Mutuality: Mutuality means that if employees leave one agency to go to another, than it is may be more likely that employees at that receiving agency will leave for the other. This may be true if the mutuality is a signal that the jobs at the agency have similar functions.
